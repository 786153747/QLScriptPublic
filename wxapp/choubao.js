...
class Task {
    ...
    async signIn() {
        ...
        if (result?.status == '200') {
            ...
            // 「请先同意协议授权」是账号侧需手动同意协议，脚本无法代签，给出指引并正常结束。
            if (/同意协议|协议.*授权|授权.*协议|需.*同意/i.test(String(msg))) {
                $.log(`🌸账号[${this.index}] 签到-失败:${msg}。需先在小程序「臭宝乐园」内手动同意《用户协议/隐私政策》后脚本才能签到，脚本无法自动代签，请到小程序里同意协议后再跑`)
            } else {
                $.log(`🌸账号[${this.index}] 签到-失败:${msg}❌`)
            }
        }
